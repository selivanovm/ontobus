...
void main()
{
	...
	bool check_control_mem(int* control_mem_addr, uint control_mem_content)
	{
		...
		{
			log.trace("main:0 !!!!!!!!!!! *control_mem_addr != control_mem_content");
			return false;
		} else
		{
			return true;
...
//        log.trace("triple_ptr={:X}", triple_ptr);
	...
	res_triples = hm.get("testkeys", null, null);
	print_triple_list(res_triples);

	log.trace("main:3 ----------------- res_triples={:X}", res_triples);
	

	for (int h=0; h < 1; h++)
	{
	
	res_triples = hm.get("testkey_1", "testkey_1", null);
	print_triple_list(res_triples);
	
	res_triples = hm.get("testkey_999", "testkey_999", null);
	print_triple_list(res_triples);

	res_triples = hm.get("testkey_99", "testkey_99", null);
	print_triple_list(res_triples);

	res_triples = hm.get("testkey_50", "testkey_50", null);
	print_triple_list(res_triples);
...
//		log.trace("print_triple_list:0 iterator={:X}", iterator);

		uint next_element = 0xFF;		
		while(next_element > 0)
		{			
			char* readed_triple = cast (char*)*iterator;
//            log.trace("print_triple_list:1 readed_triple={:X}", readed_triple);

			log.trace("print_triple_list:2 readed buff {}", str_2_char_array(readed_triple));

			next_element = *(iterator + 1);
			iterator = cast (uint*)next_element;
//			log.trace("print_triple_list:3 iterator={:X}", iterator);
		}
	}

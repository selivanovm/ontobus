...
struct this_struct_reserved_space_for_emulate_union
{
    uint16_t class_id;
    uint64_t body_size;
    void* decoded;
    ...
    struct amqp_frame_t_
{
    ...
    this_struct_reserved_space_for_emulate_union area;
}
}
extern (C) 
{
...
extern (C) 
{
    amqp_rpc_reply_t_ amqp_login(amqp_connection_state_t_* state, char* vhost, int frame_max, amqp_sasl_method_enum sasl_method,...);
}
extern (C) 
{
...
extern (C) 
{
    int amqp_basic_publish(amqp_connection_state_t_* state, amqp_bytes_t exchange, amqp_bytes_t routing_key, amqp_boolean_t mandatory, amqp_boolean_t immediate, amqp_basic_properties_t_* properties, amqp_bytes_t message_body);
}
extern (C) 
{

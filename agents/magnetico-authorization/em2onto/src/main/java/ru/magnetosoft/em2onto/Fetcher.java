...
import ru.magnetosoft.bigarch.wsclient.bl.dictionaryservice.DictionaryAttributeTEXTType;
...
import ru.magnetosoft.objects.dictionary.DictionaryAttribute;
...
import ru.magnetosoft.objects.organization.OrganizationUnit;
...
public class Fetcher {
	...
	private static QName SEARCH_QNAME = new QName(
			"http://search.bigarchive.magnetosoft.ru/", "SearchService");

	private static String DICTIONARY_URL;
	...
	private static String orgEntityServiceUrl = "/organization/OrganizationEntitySvc?wsdl";
	...
	private static void fetchDocumentTypes() {
		...
		try {
			...
			List<DocumentTemplateType> types = DocumentUtil.getInstance()
				.listDocumentTypes(DOCUMENT_URL, ticketId);

			System.out.println("Got " + types.size() + " docTypes");

			for (DocumentTemplateType documentTypeType : types) {
				System.out.println(String.format("<%s><%s><%s>.", documentTypeType.getId(), Predicates.CREATOR, documentTypeType.getAuthorId()));
			}
	...
	private static void fetchIncomingDocs() {
		List<String> docIds = getDocumentsByType(documentTypeId);
		Map<String, String> attrToProp = getPropertyMap("incomingMap.csv");
		...
		try {
			...
			int blankNumber = 0;
			String blankPrefix = "magnet-ontology#IncomingDocument";

			for (String id : docIds) {

				DocumentType type = DocumentUtil.getInstance().getDocumentById(
						DOCUMENT_URL, id, ticketId);

				String blankNode = blankPrefix + blankNumber++;
				writeTriplet(blankNode, "magnet-ontology#id", id, true, out);

				Document doc = new Document(type);
				writeTriplet(blankNode,
						"http://purl.org/dc/elements/1.1/creator", id, true,
						out);
				...
				for (TypeAttribute attr : doc.getValues()) {
					...
					String predicate = attrToProp.get(attr.getName());
					if (predicate == null) {
						predicate = "magnet-ontology#unknown_" + attr.getName();
					}

					writeTriplet(blankNode, predicate, object, true, out);
				}
			...
			System.out
					.println("Querying speed  = " + docIds.size()
							/ ((System.currentTimeMillis() - start) / 1000)
							+ " docs/s");
	...
	private static void fetchDirectives() {
		...
		try {
			...
			for (String id : docIds) {
				...
				try {
					DocumentType type = DocumentUtil.getInstance()
							.getDocumentById(DOCUMENT_URL, id, ticketId);
					...
					Document doc = new Document(type);
					writeTriplet(blankNode,
							"http://purl.org/dc/elements/1.1/creator", doc
									.getAuthor().getId(), true, out);
	...
	private static void fetchOrganization() {
		...
		try {
			...
			OrganizationUtil organizationUtil = new OrganizationUtil(properties
					.getProperty("organizationUrl"), properties
					.getProperty("organizationNameSpace"), properties
			...
			for (Department department : deps) {
				List<Department> childDeps = organizationUtil
						.getDepartmentsByParentId(department.getInternalId(),
								"Ru");
				...
				newToInternalIdMap.put(department.getInternalId(), department
						.getId());
				buCounter++;
			}
			...
			for (EntityType userEntity : list) {
				...
				for (AttributeType a : userEntity.getAttributes()
						.getAttributeList()) {
					if (a.getName().equalsIgnoreCase("firstNameRu")) {
						...
						writeTriplet(prefix, "magnet-ontology#secondName", a
								.getValue(), true, "Ru", out);
					} else if (a.getName().equalsIgnoreCase("secondnameEn")) {
						writeTriplet(prefix, "magnet-ontology#secondName", a
								.getValue(), true, "En", out);
					} else if (a.getName().equalsIgnoreCase("surnameRu")) {
						writeTriplet(prefix,
								"http://swrc.ontoware.org/ontology#lastName", a
										.getValue(), true, "Ru", out);
						...
						writeTriplet(prefix, Predicates.LOGIN_NAME, a
								.getValue(), true, out);
						...
						writeTriplet(prefix,
								"http://swrc.ontoware.org/ontology#email", a
										.getValue(), true, out);
						...
						// postCount++;
						// }
						writeTriplet(prefix, "magnet-ontology#onPosition", a
								.getValue(), true, "Ru", out);
						...
						writeTriplet(prefix, "magnet-ontology#pid", a
								.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("pager")) {
						writeTriplet(prefix, "magnet-ontology#pager", a
								.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("phone")) {
						writeTriplet(prefix,
								"http://swrc.ontoware.org/ontology#phone", a
										.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("phone")) {
						writeTriplet(prefix,
								"http://swrc.ontoware.org/ontology#phone", a
						...
						writeTriplet(prefix,
								"magnet-ontology#offlineDateBegin", a
										.getValue(), true, out);
						...
						String department = newToInternalIdMap
								.get(a.getValue());
						...
						writeTriplet(prefix, "magnet-ontology#isActive", a
								.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase(
							"employeeCategoryR3")) {
						writeTriplet(prefix,
								"magnet-ontology#employeeCategoryR3", a
										.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("r3_ad")) {
						writeTriplet(prefix, "magnet-ontology#r3_ad", a
								.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("photo")) {
						writeTriplet(prefix,
								"http://swrc.ontoware.org/ontology#photo", a
										.getValue(), true, out);
					} else if (a.getName().equalsIgnoreCase("photoUID")) {
						writeTriplet(prefix, "magnet-ontology#photoUID", a
								.getValue(), true, out);
	...
	private static void loadProperties() {
		...
		try {
			...
			DICTIONARY_URL = properties.getProperty("dictionaryUrl", "");
			fake = new Boolean(properties.getProperty("fake", "false"));
			pathToDump = properties.getProperty("pathToDump");
			orgEntityServiceUrl = properties.getProperty("pathToDump");
	...
	private static void writeDefaultProperties() {
		...
		properties.setProperty("sessionTicketId",
				"fake-tiket-4abe-8c5f-a30d6c251165");
		properties.setProperty("searchUrl",
				"http://localhost:9874/ba-server/SearchServices?wsdl");
		properties.setProperty("documentsUrl",
				"http://localhost:9874/ba-server/DocumentServices?wsdl");
		...
		properties.setProperty("organizationNameSpace",
				"http://organization.magnet.magnetosoft.ru/");
		properties
				.setProperty("organizationUrl",
						"http://localhost:9874/organization/OrganizationEntitySvc?wsdl");
		properties.setProperty("dictionaryUrl",
				"http://localhost:9874/ba-server/DictionaryServices?wsdl");
	...
	private static void printUsage() {
		System.out
				.println("Usage  : java -cp em2onto.jar ru.magnetosoft.em2onto.Fetcher [ fetchType(directive|organization) [ docType [ pathToDump [ ticketId [ searchServicesWsdl [ searchQname [ docUrl [ fake(if exists => true) ] ] ] ] ] ]");
		System.out
				.println("Example: java -cp em2onto.jar ru.magnetosoft.em2onto.Fetcher "
						+ documentTypeId
						+ " "
						+ pathToDump
						+ " "
	...
	private static boolean writeRole(String subject, AttributeType a,
		...
		if (roles.contains(a.getName())) {
			try {
				writeTriplet(subject, "magnet-ontology#" + a.getName(), a
						.getValue(), true, bw);
				return true;
			} catch (IOException e) {
				e.printStackTrace();
	...
	private static Map<String, String> getPropertyMap(String fileName) {
		...
		try {
			...
			System.out
					.println("Exception raised while reading attribute-property map from "
							+ fileName);
		}
